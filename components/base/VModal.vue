...
<template>
  <Teleport v-if="open" to="body">
    <FocusTrap :initial-focus="() => ($refs.closeButton as any)?.el">
      <component :is="is" role="dialog" aria-modal="true" :class="[open && 'is-active', size && `is-${size}`]"
        class="modal v-modal" v-bind="$attrs">
        <div class="modal-background v-modal-close" tabindex="-1" role="button"
        ...
        <div class="modal-content">
          <div class="modal-card">
            <header class="modal-card-head">
              <h3>{{ title }}</h3>
              <button ref="closeButton" class="v-modal-close ml-auto" aria-label="close" tabindex="0"
                @keydown.enter.prevent="emit('close')" @click="emit('close')">
                <VIcon icon="lucide:x" />
              </button>
            </header>
            <div class="modal-card-body" :class="[props.tabs && 'has-tabs has-slimscroll']">
            ...
            <div class="modal-card-foot" :class="[
              ...
              <slot name="cancel" :close="() => emit('close')">
                <a tabindex="0" role="button" class="button v-button v-modal-close" :class="[rounded && 'is-rounded']"
                  @keydown.enter.prevent="emit('close')" @click="emit('close')">
                  {{ cancelLabel }}
                </a>
              </slot>
